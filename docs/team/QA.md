...
| AC7 | 生命周期（agent/created 装载与库根绑定 / boot context / pre-step 提醒 / turn-stopping digest） | `lifecycle.test.mjs`：context name/order=-200 文本函数、3 类事件（不含不存在的 session-start）、pre-step 注入（plugin 消息 source 断言）+ 未触发时不注入、pre-step/turn-stopping 按 payload.agent 不串库 |
