...
| AC7 | 生命周期（`agent/session-start` + `agent/created` 兼听装载与库根绑定 / boot context / pre-step 提醒 / turn-stopping digest） | `lifecycle.test.mjs`：context name/order=-200 文本函数、**4 类事件**（v0.6.5 勘误：`agent/session-start` 真实存在，必须注册）、pre-step 注入（plugin 消息 source 断言）+ 未触发时不注入、会话视图**每会话一次**（含 clear/compact 重装）、pre-step/turn-stopping 按 payload.agent 不串库 |
